...
#: The config tables migration 0004 introduces carry an org reference too, and
#: it is an integer for the same reason the notification row's was. A host on
#: UUID keys could send notifications and still not write an org override row,
#: which is a worse state than not adopting at all: the product looks wired and
#: the admin screen silently cannot save.
